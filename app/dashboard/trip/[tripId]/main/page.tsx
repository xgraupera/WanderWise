...
const [cityWeather, setCityWeather] = useState<{ city: string; temp: number; desc: string }[]>([]);
const [mainWeather, setMainWeather] = useState<{ temp: number; desc: string } | null>(null);
...
useEffect(() => {
  async function fetchMainWeather() {
    ...
    setMainWeather({ temp: Math.round(data.main.temp), desc: data.weather[0].description });
  }
  fetchMainWeather();
...
function removeCity(index: number) {
  ...
  setCityWeather(prev => prev.filter((_, i) => i !== index));
  setHasChanges(true);
}
...
<section className="w-full bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-sm border border-gray-100 space-y-6 mt-8">
  ...
  <div className="mt-4">
    ...
    <div className="p-4 bg-white rounded-lg shadow-sm">
      ...
      {mainWeather ? (
  <p className="Capitzalize" >{mainWeather.temp}°C — {mainWeather.desc}</p>
